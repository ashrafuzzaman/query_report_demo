module QueryReportLinkHelper
  ...
  def link_to_email_query_report(target_dom_id)
    link_to t('views.labels.email'), 'javascript:void(0)', :onclick => "QueryReportEmail.openEmailModal('#{target_dom_id}');", class: 'btn' if QueryReport.config.allow_email_report
  end
end
